...
from ..base.exceptions import DataBuilderError

from ..loader import ExperimentLoader
from .metricsdf_mixin import MetricsDFStateMixin
from .results_mixin import ResultsMixin
from .morans_mixin import MoransAnalysisMixin
...
class ExperimentAnalyzer(ExperimentLoader, 
    ...
    # ========= HIDDEN METHODS ========= #
    def _get_dataorchs(self) -> dict[int | str | float, EpiDataOrchestrator]:
        """extracts the epidataorchestrators from the `dlms`"""
        if self.databuilders is None:
            raise DataBuilderError('no databuilders found')
