...
from .evaluatorplotter import EvaluationPlotter 
...
class Evaluator:
    ...
    def __init__(self, 
        ...
        # Validate that EpiConfigs are in fact compatible
        self.epiconfig = self._validate_epiconfigs()

        # Column names
        self.target_col = 'target'
        self.pred_cols = models[0].pred_col
        ...
        self.metric_calculator = MetricsCalculator(self.target_col, 
                                                   self.pred_cols, 
                                                   self.id_col, 
                                                   self.temporal_col)           
    ...
    def _compile_predictions(self, 
                             horizon: int, 
                             dataset: Literal['train','val','test']) -> pd.DataFrame:
        """
    ...
    def _compile_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for (node, modelname), group in iterator:
            ...
            if yhat.ndim == 2 and yhat.shape[1] == 1: # type: ignore
                yhat = yhat.squeeze(1)

            row  = {self.id_col: node, 
                    "model": modelname}     
                    
            for metric_name in self.metric_calculator.supported_metrics:
                metric_value: float | None  = getattr(self.metric_calculator, metric_name)(y, yhat)
    ...
    def _validate_epiconfigs(self) -> 'EpiConfig':
        ...
        if epiconfig is None: 
            raise ValueError(
                f'No valid EpiConfig found among {list(self.evaluated_models.keys())}'
                )            
